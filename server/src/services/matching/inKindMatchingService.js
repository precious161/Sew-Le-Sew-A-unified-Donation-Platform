  ...
  try {
    ...
    for (const request of pendingRequests) {
      ...
      await prisma.$transaction(async (tx) => {
        ...
        await tx.notification.create({ data: { userId: bestIntent.userId, message: `You have been matched for an In-Kind donation. A recipient needs ${requiredQuantity} unit(s) of ${request.itemType}.` }});
        await tx.notification.create({ data: { userId: request.recipientId, message: `Great news! A donor has been found for your ${request.itemType} request.` }});
      });
...
export const handleInKindDonorResponse = async (matchId, donorId, accepted) => {
  ...
  if (accepted) {
    await prisma.$transaction(async (tx) => {
      ...
      await tx.notification.create({ data: { userId: match.request.recipientId, message: `Your donor has confirmed the In-Kind donation of ${match.request.itemType}.` }});
      await tx.notification.create({ data: { userId: donorId, message: `Thank you for confirming! Please bring ${match.intent.quantity} unit(s) of ${match.intent.itemType} to ${match.intent.location}.` }});
    });
  } else {
    await prisma.$transaction(async (tx) => {
...
export const confirmInKindCompletion = async (matchId, adminId) => {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await tx.userEligibilityStatus.update({
      where: { userId_category: { userId: match.intent.userId, category: "In_Kind" } },
      data: { status: "Ineligible", ineligibleUntil: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000) },
    });
    ...
    await tx.donationHistory.create({
      data: {
        ...
        matchId: match.id,
        remarks: `Successfully donated ${match.intent.quantity} units of ${match.intent.itemType}.`,
      }
    });

    // 4. Notifications
    await tx.notification.create({ data: { userId: match.intent.userId, message: `Your In-Kind donation of ${match.intent.quantity} unit(s) of ${match.intent.itemType} has been completed and saved to your History.` }});
    await tx.notification.create({ data: { userId: match.request.recipientId, message: `Your In-Kind donation request for ${match.request.itemType} has been fulfilled.` }});
  });
};
...
export const getAllInKindMatches = async (page = 1, limit = 20) => {
  ...
  const [matches, totalCount] = await Promise.all([
    prisma.match.findMany({ skip, take: limit, where: { intent: { category: "In_Kind" } }, include: { intent: { include: { user: true } }, request: { include: { user: true } }, confirmedByUser: true }, orderBy: { createdAt: "desc" } }),
...
export const getUnmatchedInKindRequests = async (page = 1, limit = 20) => {
  ...
  const [requests, totalCount] = await Promise.all([
    prisma.donationRequest.findMany({ skip, take: limit, where: { donationType: "In_Kind", status: "Pending" }, include: { user: true }, orderBy: [{ urgencyLevel: "desc" }, { requestDate: "asc" }] }),
