...
import {
  ...
  UserCheck, UserX, AlertTriangle, Shield
} from 'lucide-react';
        ...
        <div className="p-4 md:p-8">
          <div className="md:hidden mb-6">
            ...
            <p className="text-[8px] font-bold uppercase tracking-widest text-gray-400 mt-1 italic">
              Manage {donors.length} registered donors
            </p>
          </div>
          ...
          <div className="hidden md:flex justify-between items-center mb-8">
            ...
            <div className="flex items-center gap-4">
              <button onClick={() => setBroadcastModal({isOpen: true, message: ''})} className="bg-medical-red text-white px-6 md:px-8 py-3 rounded-2xl font-black text-[10px] uppercase tracking-widest flex items-center gap-3 shadow-lg hover:bg-red-700 transition-all">
                <Radio size={16}/> Group Broadcast
              </button>
              <button onClick={fetchDonors} className="p-3 rounded-2xl bg-gray-100 dark:bg-white/5 text-gray-600 dark:text-white hover:bg-medical-red hover:text-white transition-all">
                <RefreshCw size={18}/>
              </button>
              <NotificationHub isDarkMode={isDarkMode} />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
            {filteredDonors.map((u) => (
              <div key={u.id} className={`p-5 md:p-8 rounded-[35px] md:rounded-[45px] border shadow-lg transition-all group ${isDarkMode ? 'bg-[#1e293b] border-white/5' : 'bg-white border-gray-100'}`}>
                <div className="flex justify-between items-start mb-6 text-left">
                  <div className="w-12 h-12 md:w-14 md:h-14 rounded-[20px] bg-medical-red flex items-center justify-center text-white font-black text-xl md:text-2xl shadow-inner">
                    {u.FirstName?.[0] || '?'}
                  </div>
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="px-2 md:px-3 py-1.5 md:py-2 rounded-xl bg-blue-500/10 text-blue-600 hover:bg-blue-600 hover:text-white transition-all text-[9px] md:text-[10px] font-black uppercase flex items-center gap-1"
        ...
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-[#0f172a]/95 backdrop-blur-md">
           <div className="bg-white dark:bg-[#1e293b] rounded-[40px] md:rounded-[50px] p-6 md:p-12 max-w-md w-full shadow-2xl relative mx-4">
              ...
              <div className="flex items-center gap-4 mb-6 md:mb-8">
                <div className="p-3 md:p-4 bg-medical-red text-white rounded-[20px] md:rounded-[25px] shadow-lg">
                  <Radio size={24} className="md:size-[28px]" />
                </div>
                <div>
                  <h3 className="text-xl md:text-2xl font-black text-[#1B2559] dark:text-white tracking-tighter">Group Broadcast</h3>
                  <p className="text-[9px] md:text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-1">
                    Recipients: {filteredDonors.length} Donors
                  </p>
                </div>
              </div>
              <textarea
