...
import { Users, ShieldAlert, CheckCircle, Sun, Moon, UserPlus, ShieldCheck } from 'lucide-react';
    ...
    const fetchStats = async () => {
      try {
        ...
        if (activityRes.success) {
          setStats({
            total: activityRes.totalCount,
            donors: activityRes.data.filter(u => u.Role === 'Donor').length,
            recipients: activityRes.data.filter(u => u.Role === 'Recipient').length,
    ...
    <div className={`flex min-h-screen transition-all duration-500 ${isDarkMode ? 'bg-[#0f172a]' : 'bg-[#F8F9FA]'}`}>
      ...
      <main className="flex-1 ml-72 p-10 relative overflow-y-auto h-screen">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-8">
          <StatCard icon={<Users size={22} />} label="Total Members" val={stats.total} color="bg-blue-600" dark={isDarkMode} />
          <StatCard icon={<CheckCircle size={22} />} label="Donors" val={stats.donors} color="bg-medical-red" dark={isDarkMode} />
          <StatCard icon={<UserPlus size={22} />} label="Recipients" val={stats.recipients} color="bg-green-600" dark={isDarkMode} />
          <StatCard icon={<ShieldAlert size={22} />} label="Deactivated" val={stats.inactive} color="bg-slate-700" dark={isDarkMode} />
          <StatCard icon={<ShieldCheck size={22} />} label="Pending IDs" val={stats.pendingIdentities} color="bg-yellow-500" dark={isDarkMode} />
        </div>
      </main>
    </div>
  );
