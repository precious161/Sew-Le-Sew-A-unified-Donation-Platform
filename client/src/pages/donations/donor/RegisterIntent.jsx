...
import {
  ...
  Droplets, Box, ShieldCheck, Sun, Moon, AlertTriangle, X, ArrowRight, Heart
} from 'lucide-react';
...
const RegisterIntent = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      data.append('plannedDate', new Date(formData.plannedDate).toISOString());
      if (formData.location) data.append('location', formData.location);

      if (category === 'In_Kind' || category === 'Organ') {
        if (formData.itemType) data.append('itemType', formData.itemType);
        if (category === 'In_Kind' && formData.quantity) data.append('quantity', formData.quantity);
      }

      if (category === 'Organ') {
        if (!formData.document) {
          setMessage({ type: 'error', text: 'Medical Clearance Document is required for Organ donation.' });
          setLoading(false);
          return;
        }
      ...
      const res = await DonationService.registerIntent(data);
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-[#0b1121] flex items-center justify-center p-6 transition-all duration-500">
      <div className="max-w-md w-full bg-white dark:bg-[#111C44] p-12 rounded-[50px] shadow-2xl text-center animate-in zoom-in border border-gray-100 dark:border-white/5">
        ...
        <p className="text-gray-400 text-sm mt-4 leading-relaxed italic">
          {authError === "IDENTITY_MISSING" ? "Registry policy requires a verified National ID before you can pledge." : "You must pass the medical screening quiz to enter the active donor pool."}
        </p>
        <button onClick={() => navigate(authError === "IDENTITY_MISSING" ? '/profile' : '/donations/donor/check')} className="w-full mt-10 bg-medical-red text-white py-5 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl flex items-center justify-center gap-3 hover:bg-red-700 transition-all">
  ...
  if (success) return (
    <div className="min-h-screen bg-gray-50 dark:bg-[#0b1121] flex items-center justify-center p-6 transition-all duration-500">
      <div className="max-w-md w-full bg-white dark:bg-[#111C44] p-12 rounded-[50px] shadow-2xl text-center animate-in zoom-in border border-gray-100 dark:border-white/5">
        ...
        <button onClick={() => navigate('/dashboard')} className="w-full mt-12 bg-medical-red text-white py-5 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl hover:bg-red-700 transition-all">Return to Dashboard</button>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-[#0b1121] transition-colors duration-500 pb-20 text-left">
      <div className="max-w-4xl mx-auto py-12 px-6">
        ...
        <div className="flex gap-4 mb-10 overflow-x-auto no-scrollbar pb-2">
            <button type="button" onClick={() => setCategory('Blood')} className={`px-8 py-3.5 rounded-2xl flex items-center gap-3 font-black text-[10px] uppercase tracking-widest border transition-all shrink-0 ${category === 'Blood' ? 'bg-medical-red text-white border-medical-red shadow-lg' : 'bg-white dark:bg-[#111C44] text-gray-400 border-gray-100 dark:border-white/5'}`}><Droplets size={14}/> Life Blood</button>
            <button type="button" onClick={() => setCategory('In_Kind')} className={`px-8 py-3.5 rounded-2xl flex items-center gap-3 font-black text-[10px] uppercase tracking-widest border transition-all shrink-0 ${category === 'In_Kind' ? 'bg-medical-red text-white border-medical-red shadow-lg' : 'bg-white dark:bg-[#111C44] text-gray-400 border-gray-100 dark:border-white/5'}`}><Box size={14}/> Medical Supplies</button>
            <button type="button" onClick={() => setCategory('Organ')} className={`px-8 py-3.5 rounded-2xl flex items-center gap-3 font-black text-[10px] uppercase tracking-widest border transition-all shrink-0 ${category === 'Organ' ? 'bg-medical-red text-white border-medical-red shadow-lg' : 'bg-white dark:bg-[#111C44] text-gray-400 border-gray-100 dark:border-white/5'}`}><Heart size={14}/> Organ Intent</button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white dark:bg-[#111C44] p-10 rounded-[55px] shadow-2xl border border-gray-100 dark:border-white/5 transition-all">
          <div className="space-y-12">
             ...
             {category === 'Organ' && (
               <div className="grid grid-cols-1 md:grid-cols-2 gap-10 animate-in slide-in-from-left duration-300">
                  <div className="space-y-3">
                    <label className="text-[10px] font-black uppercase text-gray-400 dark:text-white/40 ml-2 block text-left">Organ to Donate</label>
                    <input placeholder="e.g. Kidney" required value={formData.itemType} onChange={(e) => setFormData({...formData, itemType: e.target.value})} className="w-full p-5 rounded-[22px] bg-gray-50 dark:bg-[#0b1121] border-none outline-none font-bold text-sm text-[#111C44] dark:text-white shadow-inner" />
                  </div>
                  <div className="space-y-3">
                    <label className="text-[10px] font-black uppercase text-gray-400 dark:text-white/40 ml-2 block text-left">Medical Clearance (PDF/JPG)</label>
                    <input type="file" required onChange={(e) => setFormData({...formData, document: e.target.files[0]})} className="w-full p-4 rounded-[22px] bg-gray-50 dark:bg-[#0b1121] text-[10px] text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:bg-medical-red file:text-white file:font-black shadow-inner" />
                  </div>
               </div>
             )}

             <div className="p-8 rounded-[35px] bg-blue-500/5 border border-blue-500/10 flex gap-6 items-center">
                <div className="w-12 h-12 bg-white dark:bg-white/5 rounded-2xl flex items-center justify-center text-blue-500 shadow-sm border border-blue-100 dark:border-white/5 transition-all shrink-0"><ShieldCheck size={24} /></div>
