...
const HealthInfo = () => {
  ...
  // 1. PERSISTENCE LOGIC: Call /medical-profile to fill the form
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        console.log("Initialize Mode: No profile found yet.");
      } finally {
        setFetching(false);
      }
    };
      ...
      <div className="max-w-4xl mx-auto py-12 px-6">
        ...
        <div className="mb-10 flex justify-between items-center">
          <button 
            ...
            className="flex items-center gap-2 text-[#111C44] dark:text-white/50 hover:text-blue-600 transition-all group"
          >
          ...
          <button onClick={toggleTheme} className="p-3 rounded-2xl bg-white dark:bg-white/5 text-[#111C44] dark:text-yellow-400 border border-gray-200 dark:border-white/10 shadow-xl transition-all active:scale-95">
        ...
        >
          ...
          <button type="submit" disabled={loading}
            className="w-full bg-[#111C44] dark:bg-medical-red text-white py-6 rounded-[25px] font-black text-xs uppercase tracking-[0.4em] shadow-2xl transition-all active:scale-95 disabled:opacity-50">
