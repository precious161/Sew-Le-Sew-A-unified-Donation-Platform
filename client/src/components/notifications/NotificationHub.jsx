// src/components/notifications/NotificationHub.jsx
...
const NotificationHub = ({ isDarkMode }) => {
  ...
  const handleMarkAllRead = async () => {
    const res = await AlertService.markAllAsRead();
    if (res.success) refreshList();
  };
      ...
      <button
        ...
        className={`p-3 rounded-2xl transition-all relative ${
          isDarkMode
            ...
            : 'bg-white shadow-sm text-gray-400 hover:text-medical-red border border-gray-100'
        }`}
      >
        <Bell size={20} />
        {unreadCount > 0 && (
      ...
      {isOpen && (
        <div className={`absolute right-0 mt-4 w-80 rounded-[30px] shadow-2xl border transition-all animate-in fade-in slide-in-from-top-2 z-[100] ${
          ...
          <div className="p-6 border-b border-white/5 flex justify-between items-center">
            <h3 className={`font-black uppercase italic tracking-tighter ${isDarkMode ? 'text-white' : 'text-[#1B2559]'}`}>
              Updates
            </h3>
            <div className="flex items-center gap-3">
              {unreadCount > 0 && (
                <button
                  ...
                  className="text-[9px] font-black uppercase tracking-widest text-medical-red hover:text-red-700 flex items-center gap-1"
                >
          ...
          <div className="max-h-[400px] overflow-y-auto custom-scrollbar">
            {notifications.length === 0 ? (
              ...
              notifications.map((n) => (
                <div
                  key={n.id}
                  className={`p-5 border-b border-white/5 flex gap-4 transition-all ${
                    !n.isRead ? 'bg-medical-red/5' : 'opacity-40'
                  }`}
                >
                  <div className={`mt-1 p-2 h-fit rounded-lg ${
                    !n.isRead ? 'bg-medical-red text-white' : 'bg-gray-100 text-gray-400'
                  }`}>
                    <Info size={14} />
                  </div>
